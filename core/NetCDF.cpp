...
RegScalarField3f* NetCDF::ImportQGOmega(const std::string& path, const std::string& varname) {
	// get the info object
	NetCDF::Info info;
	if (!ReadInfo(path, info)) return NULL;
	...
	// read the resolution from the info object
	const NetCDF::Info::Variable& variable = info.GetVariableByName(varname);
	size_t resX = variable.GetDimensionByName("dimx_QX").GetLength();
	size_t resY = variable.GetDimensionByName("dimy_QX").GetLength();
	size_t resZ = variable.GetDimensionByName("dimz_QX").GetLength();
	...
	//ImportFloatArray(path, dimYname, dimY);
	//ImportFloatArray(path, dimZname, dimZ);
	domain.ExpandByPoint(Vec3d({ -180, -90, 200 }));
	domain.ExpandByPoint(Vec3d({ 180, 90, 1000 }));
