...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-pMin") {
            ...
            i++;
            if (i < argc) {
                jet_params.max_steps_below_speed_thresh = atoi(argv[i]);
            }
            else {
                std::cout << "Not enough arguments." << std::endl;
            }
            ...
            i++;
            if (i < argc) {
                jet_params.n_predictor_steps = atoi(argv[i]);
            }
            else {
                std::cout << "Not enough arguments." << std::endl;
            }
            ...
            i++;
            if (i < argc) {
                jet_params.n_corrector_steps = atoi(argv[i]);
            }
            else {
                std::cout << "Not enough arguments." << std::endl;
            }
    ...
    for (const auto& time_step : time_steps)
    {
        ...
        const LineCollection& jet = jet_stream->GetJetCoreLines();
        if (export_txt)
        {
