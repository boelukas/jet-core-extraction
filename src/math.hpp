...
public:
	...
	struct PointCloud {
		std::vector<Vec> pts;
		inline bool is_empty() const { return pts.size() == 0; }
		inline size_t kdtree_get_point_count() const { return pts.size(); }
		inline TScalarType kdtree_get_pt(const size_t idx, const size_t dim) const
		{
			return pts[idx][dim];
		}
		template <class BBOX>
...
typedef std::vector<Vec3d> Line3d;
typedef std::vector<Vec3d> Points3d;
typedef std::unordered_set<Vec3d, Vec3d::VectorHash> Vec3dSet;
typedef std::set<Vec3d> Vec3dOrderedSet;
typedef Vec3d::PointCloud PointCloud3d;
...
public:
	static float IndexOfValueInArray(const std::vector<float>& arr, const float& val, const bool& flipped_axis) {
		...
		for (int i = 0; i < arr.size(); i++) {
			if (val < arr[i]) {
				int down = std::max(i - 1, 0);
				int up = std::min(i, (int)arr.size() - 1);
				float val_up = arr[up];
				float val_down = arr[down];
				temp_res = down + (up - down) / (val_up - val_down) * (val - val_down);
				break;
			}
			else if (val == arr[i]) {
				temp_res = (float)i;
	...
	static float ValueOfIndexInArray(const std::vector<float>& arr, const float& index, const bool& flipped_axis) {
		...
		int ind_up;
		if (flipped_axis) {
			ind_down = res_axis - 1 - (int)std::ceil(index);
			ind_up = res_axis - 1 - (int)std::floor(index);
		}
		else {
			ind_down = (int)std::ceil(index);
			ind_up = (int)std::floor(index);
		}
		float val_up = arr[ind_up];
		float val_down = arr[ind_down];

		if (flipped_axis) {
			return val_down + (val_up - val_down) * (res_axis - 1 - index - ind_down);
		}
		else {
			return val_down + (val_up - val_down) * (index - ind_down);
		}
	}
